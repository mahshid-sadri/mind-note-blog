...
export const client = createClient({
  projectId: 'n9inbpje', 
  dataset: 'production',
  useCdn: false, // Set to false to bypass CDN caching issues on new projects
...
export const sanityService = {
  ...
  getAllPosts: async (): Promise<BlogPost[]> => {
    try {
      ...
      console.error("Sanity Fetch Error: (Potential CORS issue). Using fallback data.", error);
      return ALL_POSTS; // Graceful fallback on error
    }
  },
